...
static int insert_user(ctx *c, user *insert_user) {
  ...
  json_t *context = json_pack("{s:s,s:s,s:i,s:s,s:s}", "id", insert_user->id, "name", insert_user->name, "age",
                              insert_user->age, "email", insert_user->email, "password", insert_user->password);

  char *query = NULL;
  size_t query_size = 0;
  int result_status =
      mustach_jansson_mem((const char *)insert_user_sql, insert_user_sql_len, context, 0, &query, &query_size);
  json_decref(context);
  if (result_status < 0) {
    WARN_PRINTLN("user_repository insert_user failed template");
    return -1;
  }
  ...
  free(query);
  if (result_status != 0) {
    WARN_PRINT("user_repository insert_user exec query\n");
    const char *error_message = mysql_error(conn);
    WARN_PRINT("error_message : %s\n", error_message);
...
static int delete_user(ctx *c, const char *id) {
  ...
  size_t query_size = 0;
  int result_status =
      mustach_jansson_mem((const char *)delete_user_sql, delete_user_sql_len, context, 0, &query, &query_size);
  json_decref(context);
  if (result_status < 0) {
    WARN_PRINT("user_repository delete_user failed template\n");
    return -1;
  }
  result_status = mysql_query(c->mysql_connection, query);
  free(query);
  if (result_status != 0) {
    WARN_PRINT("user_repository delete_user failed exec query\n");
    const char *err_msg = mysql_error(c->mysql_connection);
    WARN_PRINT("err_msg : %s\n", err_msg);
    free((void *)err_msg);
    return -1;
  }
  return 0;
...
static user *get_user(ctx *c, const char *id) {
  ...
  size_t query_size = 0;
  int result_status =
      mustach_jansson_mem((const char *)get_user_sql, get_user_sql_len, context, 0, &query, &query_size);
  json_decref(context);
  if (result_status != 0) {
    WARN_PRINT("user_repository get_user failed template\n");
    return NULL;
  }
  DEBUG_PRINT("get query : %s\n", query);

  result_status = mysql_query(c->mysql_connection, query);
  free(query);
  if (result_status != 0) {
    WARN_PRINT("user_repository get_user failed exec query\n");
    const char *err_msg = mysql_error(c->mysql_connection);
    WARN_PRINT("err_msg is %s\n", err_msg);
    free((void *)err_msg);
    return NULL;
  }
