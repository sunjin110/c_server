...
extern mysql_connection_pool *new_mysql_connection_pool(mysql_config *config, int connection_count) {
  mysql_connection_pool *pool = (mysql_connection_pool *)malloc(sizeof(mysql_connection_pool));
...
extern void append_mysql_connection(mysql_connection_pool *pool, MYSQL *mysql_client) {
  mysql_connection_element *new_element = (mysql_connection_element *)malloc(sizeof(mysql_connection_element));
...
extern MYSQL *pop_mysql_connection(mysql_connection_pool *pool) {
  if (pool == NULL) {
    DEBUG_PRINT("mysql_connections is NULL\n");
    return NULL;
  }
  ...
  DEBUG_PRINT("mysql_client pop label_num:%d\n", pop_element->label_num);
